...
const standardRatios = [[1, 1], [4, 3], [3, 4], [5, 4], [4, 5], [3, 2], [2, 3], [16, 10], [10, 16], [16, 9], [9, 16], [21, 9], [9, 21], [32, 9], [9, 32]].reduce((out, [x, y]) => Object.assign({}, out, {
  [x / y]: [x, y]
...
const controls = {
  ...
  updateSeekTooltip(event) {
    ...
    const point = (_this$config$markers = this.config.markers) == null ? void 0 : (_this$config$markers$ = _this$config$markers.points) == null ? void 0 : _this$config$markers$.find(({
      time: t
  ...
  setMarkers() {
    ...
    const points = (_this$config$markers2 = this.config.markers) == null ? void 0 : (_this$config$markers3 = _this$config$markers2.points) == null ? void 0 : _this$config$markers3.filter(({
      time
...
class Fullscreen {
  constructor(player) {
    ...
    this.toggleFallback = (toggle = false) => {
...
const ui = {
  ...
  migrateStyles() {
    ...
    Object.values(Object.assign({}, this.media.style)) // We're only fussed about Plyr specific properties
  ...
  constructor(_player) {
    ...
    this.media = () => {
      ...
      if (player.supported.ui && player.config.clickToPlay && !player.isAudio) {
        ...
        const {
          wrapper
        } = player.elements; // Bail if there's no wrapper (this should never happen)
    ...
    this.proxy = (event, defaultHandler, customHandlerKey) => {
      const {
        player
      } = this;
    ...
    this.bind = (element, type, defaultHandler, customHandlerKey, passive = true) => {
      const {
        player
      } = this;
      const customHandler = player.config.listeners[customHandlerKey];
  ...
  ready() {
    ...
    const {
      ...
      referrerPolicy
          ...
          frameParams = _objectWithoutPropertiesLoose(config, _excluded); // Get the source URL or ID
    ...
    const params = buildUrlParams(Object.assign({
...
class Ads {
  ...
  constructor(player) {
    ...
    this.pollCountdown = (start = false) => {
      if (!start) {
    ...
    this.startSafetyTimer = (time, from) => {
  ...
  constructor(player) {
    ...
    this.startMove = event => {
      ...
      if (event.type === 'touchmove') {
        ...
        const point = (_this$player$config$m = this.player.config.markers) == null ? void 0 : (_this$player$config$m2 = _this$player$config$m.points) == null ? void 0 : _this$player$config$m2.find(({
          time: t
    ...
    this.showImage = (previewImage, frame, qualityIndex, thumbNum, thumbFilename, newImage = true) => {
    ...
    this.removeOldImages = currentImage => {
    ...
    this.preloadNearby = (thumbNum, forward = true) => {
      return new Promise(resolve => {
        setTimeout(() => {
    ...
    this.getHigherQuality = (currentQualityIndex, previewImage, frame, thumbFilename) => {
    ...
    this.toggleThumbContainer = (toggle = false, clearShowing = false) => {
    ...
    this.toggleScrubbingContainer = (toggle = false) => {
...
class Plyr {
  constructor(target, options) {
    ...
    this.rewind = seekTime => {
      this.currentTime -= is.number(seekTime) ? seekTime : this.config.seekTime;
    };

    this.forward = seekTime => {
      this.currentTime += is.number(seekTime) ? seekTime : this.config.seekTime;
    };
    ...
    this.destroy = (callback, soft = false) => {
      if (!this.ready) {
        return;
      }
    ...
    this.supports = type => support.mime.call(this, type);
